...
def test_dataloader(**params):
    """ """
    dataloader = DataLoader(mode='train', **params)
    trainloader, validloader = dataloader.dataset.get_train_valid_loader(show_sample=True)

    num_train_samples = len(trainloader.sampler)
    num_valid_samples = len(validloader.sampler)

    # dataloader iterator to get next sample
    dataiter = iter(trainloader)
    sample_data, sample_target = dataiter.next()

    print(f'\nSample train batch data size : ', size_str(sample_data))
    print(f'Sample train batch target size : ', size_str(sample_target))
    print(f'\nNumber of training samples : {num_train_samples}')
    print(f'Number of validation samples : {num_valid_samples}')


    dataloader = DataLoader(mode='test', **params)
    testloader = dataloader.get_test_loader()
    num_test_samples = len(testloader.sampler)

    # dataloader iterator to get next sample
    dataiter = iter(testloader)
    sample_data, sample_target = dataiter.next()

    print(f'\nSample test batch data size : ', size_str(sample_data))
    print(f'Sample test batch target size : ', size_str(sample_target))
