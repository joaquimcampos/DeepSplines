...
def plot_activations(params):
    ...
    if params['plot_activation_input'] == True and ckpt['params']['net'] != 'twoDnet_onehidden':
        raise ValueError('Should use twoDnet_onehidden for plot_activation_input option')
    ...
    if params['plot_activation_input']:
        x_min, x_max = get_input_statistics(params['ckpt_filename'])
    ...
    for i, activation_layer in enumerate(activations_list):
        ...
        for i in range(activ.shape[0]):
            ...
            if params['plot_activation_input']:
                assert params['yrange'] is not None
                y_range = params['yrange'][0:2] if (i <= 1) else params['yrange'][2:]
                vlen = 0.08 * (y_range[1] - y_range[0])
                plt.vlines(x=x_min[i], ymin=y_range[0], ymax=y_range[0] + vlen, color=first_col)
                plt.vlines(x=x_max[i], ymin=y_range[0], ymax=y_range[0] + vlen, color=first_col)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--yrange', metavar='yrange', type=float, nargs=4, help='set y axis limit')
    ...
    parser.add_argument('--plot_activation_input', action='store_true', help='Check where data falls.')
